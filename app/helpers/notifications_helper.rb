  ...
  def object_link(note, actors)
    ...
    if note.instance_of?(Notifications::Mentioned)
      if post = note.linked_object
        translation(target_type, :actors => actors, :count => actors_count, :post_link => link_to(t('notifications.post'), post_path(post), :title => post.hint).html_safe)
      else
      ...
      if post = note.linked_object
        translation(
          ...
          :actors => actors,
          :count => actors_count,
          :post_author => h(post.author.name),
          :post_link => link_to(
            t('notifications.post'),
            post_path(post),
            'data-ref' => post.id,
            :class => 'hard_object_link',
            :title => post.respond_to?(:hint) ? post.hint : nil
          ).html_safe
        )
      else
        t(note.deleted_translation_key, :actors => actors, :count => actors_count).html_safe
